...
def draw_genmat(img, name):
    ...
    plt.savefig(f'./results_gauss/genomat_{name}.png')
    plt.show()
...
class MCMCGAN():
    ...
    def hist_samples(self, it, bins=10):
        ...
        plt.savefig(f'./results_gauss/mcmcgan_hmc_it{it}.png')
    ...
    def traceplot(self, it, ylim, c='r', a=.3):
        ...
        plt.savefig(f'./results_gauss/mcmcgan_trace_it{it}.png')
        plt.show()
...
def plot_average(x, y, param_name, name, log_scale, bins=10):
  ...
  plt.ylim((0, 1))
  plt.savefig(f'./results_gauss/{name}.png')
  plt.clf()
...
while convergence == False and max_num_iters != it:
    ...
    new_params = new_params[new_params > mcmcgan.genob.param_bounds[0]]
    mcmcgan.initial_guess = tf.constant([sample_mean.numpy()])

    sns.distplot(new_params, bins=10)
    plt.title(f"MCMC Samples distribution - Gauss({sample_mean.numpy()}, {sample_stddev.numpy()})")
    plt.legend(['MCMC samples', 'Gaussian approx.'])
    plt.savefig(f'./results_gauss/mcmcgan_{mcmcgan.kernel_name}_it{it}.png')
    plt.show()
...
ridge_plot.savefig('./results_gauss/ridgeplot.png')
